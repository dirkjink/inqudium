...
public final class InqCompatibility {
  ...
  private InqCompatibility(Map<InqFlag, Boolean> flags) {
    this.flags = Collections.unmodifiableMap(new EnumMap<>(
        flags.isEmpty() ? Collections.emptyMap() : flags));
  }
  ...
  public static final class Builder {
      ...
      try {
        ...
        for (var provider : loader) {
          try {
            ...
            System.getLogger(InqCompatibility.class.getName())
                .log(System.Logger.Level.WARNING,
                    "Failed to load InqCompatibilityOptions provider: {0}", e.getMessage());
          }
        }
      } catch (Exception e) {
        System.getLogger(InqCompatibility.class.getName())
            .log(System.Logger.Level.WARNING,
                "ServiceLoader discovery for InqCompatibilityOptions failed: {0}", e.getMessage());
      }
    ...
    private void mergeFlags(EnumMap<InqFlag, Boolean> target, InqCompatibilityOptions provider) {
      try {
        ...
        System.getLogger(InqCompatibility.class.getName())
            .log(System.Logger.Level.WARNING,
                "InqCompatibilityOptions.flags() threw: {0}", e.getMessage());
      }
    }
  }
}
