 ...
 * "no preset baseline"). The record is paradigm-agnostic: a bulkhead's paradigm is carried by
 * its handle and live container, never by the snapshot.
 *
 ...
 * @param strategy             strategy choice for the bulkhead's permit-management algorithm
 *                             (ADR-032). Non-null; the documented default — applied by every
 *                             paradigm's snapshot baseline — is {@link SemaphoreStrategyConfig},
 *                             matching the historical "no special strategy" semantics. The
 *                             paradigm's hot-phase materialization picks the concrete strategy
 *                             from this field at cold-to-hot transition time.
 */
