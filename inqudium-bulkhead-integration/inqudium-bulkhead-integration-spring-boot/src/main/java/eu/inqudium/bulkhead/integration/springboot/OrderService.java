...
import static eu.inqudium.bulkhead.integration.springboot.BulkheadConfig.BULKHEAD_NAME;

/**
 * Tiny webshop order service used as the example domain.
 *
 * <p>The class is plain Java decorated with {@link Service @Service} so Spring Boot's
 * component scan picks it up automatically. Resilience attaches at runtime through Spring
 * AOP: each method that should be protected carries the {@link InqBulkhead} annotation from
 * {@code inqudium-annotation} — the same annotation the AspectJ-native and plain-Spring
 * examples use — and Spring's proxy generator wraps the bean with a dispatch surface that
 * routes annotated methods through {@link eu.inqudium.spring.InqShieldAspect}.
 *
 * <p>The aspect is not woven into the bytecode and the call site does not build a proxy
 * manually: Spring builds the proxy when it instantiates the bean. In Spring Boot the two
 * preconditions are met implicitly — {@code spring-boot-starter-aspectj} on the classpath
 * activates {@code @EnableAspectJAutoProxy} via Boot's AOP auto-configuration, and
 * {@link eu.inqudium.spring.boot.InqAutoConfiguration} discovers Inqudium's
 * {@link eu.inqudium.spring.InqShieldAspect} bean and registers it. Compare with the
 * plain-Spring example, where both opt-ins are performed by hand on the configuration class.
 *
 ...
 * calls each method from outside the bean (from {@link OrderServiceApplication} and from the
 * tests), so the caveat does not bite here; a production service that needs intra-bean calls
 * would either inject {@code @Lazy OrderService self} or split the protected methods into a
 * separate bean. The Javadoc on {@link eu.inqudium.spring.InqShieldAspect} discusses the
 * workarounds.
 */
@Service
public class OrderService {
     ...
     * @param release  awaited before the body returns; the test (or {@link
     *                 OrderServiceApplication}) signals this latch when the permits should be
     *                 freed.
     ...
     * — the surface through which the rejection reaches the caller is documented in
     * {@link OrderServiceApplication#demonstrateAsyncSaturation}.
     *
